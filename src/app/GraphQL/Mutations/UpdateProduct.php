...
class UpdateProduct
{
    public function __invoke($rootValue, array $args)
    {
        ...
        if (isset($args['image_url'])) $updateData['image_url'] = $args['image_url'];
        if (isset($args['is_active'])) $updateData['is_active'] = $args['is_active'];

        $product->update($updateData);

        // TODO: Dispatch job to update product in Elasticsearch
        // dispatch(new UpdateProductIndex($product));
